...
def simulate(host_type, host_number, 
             max_bw, miner_percentages, 
             transaction_counts, simulation_count,
             root_path, debug_mode):
    ...
    for miner_percentage, transaction_count \
        ...
        for i in range(0, simulation_count):
            ...
            parametered_path = ('h' + str(host_number) + 
                                'm' + str(miner_percentage) + 
            ...
            transactor_names = [random.choice(topo.hosts()) for x 
                                in range(transaction_count)]
            subsimulation(net_params, miner_names, 
                          transactor_names, path, 
                          debug_mode)

def subsimulation(net_params, miner_names,
                  transactor_names, sim_path,
                  debug_mode):
    net = None
    try:
        start_time = time()
        ...
        ts_dir_path = init_simulation_path(sim_path + '/' + 
                                           timestamp_str + '/')
        ...
        transactors = [net.getNodeByName(host_name)
                       for host_name in transactor_names]
...
def main():
    ...
    help_message = 'blockchain consensus class to be used'
    parser.add_argument('-ht', '--host_type', 
                        default='pow', type=str, 
                        help=help_message)
    ...
    help_message = 'debug mode for xterms.'
    parser.add_argument('-d', '--debug',
                        default=False, help=help_message,
                        action='store_true')
        
    help_message = 'go with setup mode for h=10,20,50,100 m=10,20,50,80 tx=10, sim=10'
    ...
    if args.setup:
        ...
        simulate(host_type, host_number, 
                 ...
                 [transaction_count], simulation_count,
                 args.path, args.debug)
