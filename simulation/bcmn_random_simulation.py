...
def simulate(
    ...
    for miner_percentage, transaction_count in itertools.product(
            miner_percentages, transaction_counts):
        for i in range(0, simulation_count):
            print("Simulating with miner %: " + str(miner_percentage)
                  + ", transaction #: " + str(transaction_count))
            parametered_path = 'h' + \
                str(host_number) + 'm' + str(miner_percentage) + 't' + str(transaction_count)
            ...
            transactor_names = [
                random.choice(
                    topo.hosts()) for x in range(transaction_count)]
            subsimulation(
                net_params,
                miner_names,
                transactor_names,
...
def main():
    ...
    parser.add_argument(
        ...
        default='pow',
        type=str,
        help='blockchain consensus class to be used')
    parser.add_argument(
        '-p',
        '--path',
        default='/tmp/',
    ...
    parser.add_argument(
        ...
        '--debug',
        default=False,
        help='debug mode for xterms.',
        action='store_true')
    parser.add_argument(
        '-s',
        '--setup',
        default=False,
        help='go with setup mode for h=10,20,50,100 m=10,20,50,80 tx=10, sim=10',
        action='store_true')
