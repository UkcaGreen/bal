...
class POWBlockchain(BaseBlockchain):
    def genesis_block(self):
        return self.raw_block(0, 1465154705, '', 
                              ...
                              self.get_initial_difficulty(), 0)

    def raw_block(self, index, timestamp, 
    ...
    def find_block(self, index, previous_hash, transactions, difficulty):
        ...
        timestamp = time()
        while (True):
            temp_block = self.raw_block(index, timestamp, previous_hash, 
                                        transactions, difficulty, nonce)
    ...
    def is_valid_block_structure(self, block):
        return (isinstance(block['index'], numbers.Number) 
                ...
                and isinstance(block['proof'], numbers.Number))
